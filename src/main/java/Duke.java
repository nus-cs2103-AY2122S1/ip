...
public class Duke {
    ...
    private static void handleCommand(String command, boolean silent) throws IrisException {
        if (command.equals("list")) {
            listTasks();
        } else if (command.startsWith("done")) {
            Task task = done(parseInt(getMetadata(command)));
            if (!silent) say(String.format("Good job! I've marked this task as done: %s", task));
