...
public class Duke {
    ...
    public void dukeBye() throws DukeException{
        Ui.showByeMessage();
        try {
            storage.save(tasks);
        } catch (IOException e) {
    ...
    public void dukeList() {
        tasks.list();
    }

    public void dukeDone(int i) throws DukeException {
        tasks.markAsDone(i);
    }

    public void dukeDelete(int i) throws DukeException {
        tasks.delete(i);
    }

    public void dukeTodo(String desc) {
        Todo todo = new Todo(desc);
        tasks.add(todo);
        Ui.showAddTaskMessage(todo.toString(), tasks.size());
    }

    public void dukeDeadline(String desc, Calendar cal){
        Deadline deadline = new Deadline(desc, cal);
        tasks.add(deadline);
        Ui.showAddTaskMessage(deadline.toString(), tasks.size());
    }

    public void dukeEvent(String desc, Calendar cal){
