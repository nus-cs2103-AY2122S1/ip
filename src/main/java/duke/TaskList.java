...
public class TaskList implements Serializable {
     ...
     * @param task Task to add to the list.
     */
    public void addTaskCli(Task task) {
        taskList.add(task);
    ...
    /**
     * Delete a task by its index. Print the task name and total task amount in the list after adding. For command line.
     *
     * @param index Index of the task to be deleted.
     */
    public void deteteTaskCli(int index) {
    ...
    /**
     * Search if there are tasks with a given keyword for command line
     *
     * @param keyword keyword to search
     * @return searched list
     */
    public void findCli(String keyword) {
        ArrayList<Task> outList = new ArrayList<Task>();
        for (int i = 0; i < taskList.size(); i++) {
            if (taskList.get(i).name().indexOf(keyword) >= 0) {
                outList.add(taskList.get(i));
