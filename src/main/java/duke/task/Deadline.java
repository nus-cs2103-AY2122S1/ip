...
public class Deadline extends Task {
    ...
    public String getSaveInfo() {
        if (this.isDone()) {
            return String.format("D | 1 | %s | %s | %s", this.getTaskName(), this.getWhen(), super.getTagsInString());
        } else {
            return String.format("D | 0 | %s | %s | %s", this.getTaskName(), this.getWhen(), super.getTagsInString());
        }
    }
}
